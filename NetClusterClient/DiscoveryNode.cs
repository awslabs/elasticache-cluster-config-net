...
namespace NetClusterClient
{
    ...
    public class DiscoveryNode
    {
        ...
        public string GetNodeConfig()
        {
            ...
            if (nodeVersion.CompareTo(older) < 0)
            {
                while (waiting && tries > 0)
                {
                    ...
                    try
                    {
                        log.Debug("Node version older than 1.4.14");
                        IGetOperation command = new GetOperation("AmazonElastiCache:cluster");
                ...
                if (waiting)
                {
                    throw new TimeoutException(String.Format("Could not get config of version lower than 1.4.14. Tries: {0} Delay: {1}. " + message, this.tries, this.delay));
                }
            }
            else
            {
                while (waiting && tries > 0)
                {
                    ...
                    try
                    {
                        log.Debug("Node version 1.4.14 or newer");
                        ConfigGetOperation command = new ConfigGetOperation("cluster");
                ...
                if (waiting)
                {
                    throw new TimeoutException(String.Format("Could not get config of 1.4.14 or higher. Tries: {0} Delay: {1}. " + message, this.tries, this.delay));
                }
            }
